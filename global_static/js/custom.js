...
// ===================================
	...
	if (event.target.classList.contains('btn-del-product')) {
		...
		fetch(URL_DELCART, createAjaxParams({id: id}))
			.then(res => res.json())
			.then(data => {
				const response = data['data'];
				const sums = data['sums']
	...
	quickviewPopupLinks.forEach((link) => {
		link.addEventListener('click', function (event){
			...
			.then(responseData => {
				...
				for (let q = 0; q< colMd5Elements.length; q++) {
					let colMd5Element = colMd5Elements[q];

					let imgElements = colMd5Element.getElementsByTagName("img");
				...
				let text_list = [
					{'class': "box-title", 'text': responseData['name']},
					{'class': "box-price", 'text': responseData['price'] + ' $'},
					{'class': "box-text", 'text': responseData['description']},
					{'class': "box-text", 'text': responseData['description']},
				]

				for (let i= 0; i < text_list.length; i++){
					updateText(text_list[i]['class'], text_list[i]['text'])
				}
				...
				for (let i = 0; i < stocks.length; i++) {
					if (responseData['available'] === true) {
					  stocks[i].innerText = "В наличии";
					} else {
					  stocks[i].innerText = "Отсутствует";
					}
				}
    ...
    sortingSelect.forEach((link) => {
		link.addEventListener('click', function (event){
			...
			fetch(URL_SORT, createAjaxParams({ sort: selectedValue }))
			.then(res => res.json())
			.then(data => {
				const response = data['data'];
						...
						<div class="product-box common-cart-box box">
							<div class="item product-shop" data-id="${response[k]['pk']}">
								...
								<div class="product-info common-cart-info text-center">
									...
									<p class="product-list-text" style="">${response[k]['fields']['description']}</p>
									<div class="hover-option">
										<div class="add-cart-btn">
	...
	$('.quickview-popup-link').magnificPopup({
		...
		callbacks: {
			open: function () {
				...
				if ($(window).width() >= 768) {
					var firstImgHeight = $(".quickview-popup-link .product_img").height();
					var divWidth = $(".quickview-product-detail").width();
					$(".quickview-popup .product_img").elevateZoom({
						cursor: "crosshair",
						easing: true,
